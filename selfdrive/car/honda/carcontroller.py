...
class CarController():
  def __init__(self, dbc_name, CP, VM):
    ...
    self.vision_turn_control_enabled = Params().get_bool("TurnVisionControl")
    self.turn_speed_control_enabled = Params().get_bool("TurnSpeedControl")
  ...
  def update(self, enabled, active, CS, frame, actuators, pcm_cancel_cmd,
    ...
    if not CS.CP.openpilotLongitudinalControl:
      ...
      if not CS.out.cruiseState.standstill and (enabled and CS.acc_active) and self.speed_limit_control_enabled:
        cruise_button = self.get_cruise_buttons(CS)
        if cruise_button is not None:
          can_sends.append(hondacan.spam_buttons_command(self.packer, cruise_button, idx, CS.CP.carFingerprint))
          self.resume_count += 1
        else:
          self.resume_count = 0
  ...
  def get_speed_limit_osm(self):
    self.sm.update(0)
    speed_limit_osm = float(self.sm['longitudinalPlan'].speedLimit if self.sm['longitudinalPlan'].speedLimit is not None else 0.0) * CV.MS_TO_KPH
  ...
  def get_speed_limit_offset_osm(self):
    ...
    if speed_limit_perc_offset:
      self.sm.update(0)
      speed_limit_offset = float(self.sm['longitudinalPlan'].speedLimitOffset) * CV.MS_TO_KPH
    else:
      speed_limit_offset = (float (speed_limit_value_offset * CV.MPH_TO_KPH)) if not is_metric else speed_limit_value_offset
  ...
  def get_curve_speed(self, target_speed_kph):
    ...
    if self.vision_turn_control_enabled:
      self.sm.update(0)
      ...
      if int(vision_v_cruise_kph) == int(v_cruise_kph_prev):
        ...
        vision_v_cruise_kph = min(target_speed_kph, vision_v_cruise_kph - 8)
    else:
      vision_v_cruise_kph = 255
    if self.turn_speed_control_enabled:
      self.sm.update(0)
  ...
  def get_cruise_buttons(self, CS):
    ...
    if not self.get_cruise_buttons_status(CS):
      ...
      if self.vision_turn_control_enabled or self.turn_speed_control_enabled:

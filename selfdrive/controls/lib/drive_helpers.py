...
def update_v_cruise(v_cruise_kph, buttonEvents, enabled, cur_time, accel_pressed,decel_pressed,accel_pressed_last,decel_pressed_last, fastMode):
  ...
  if enabled:
    if accel_pressed:
      if ((cur_time-accel_pressed_last) >= 0.5 or (fastMode and (cur_time-accel_pressed_last) >= 1)):
        v_cruise_kph += 1
    elif decel_pressed:
      if ((cur_time-decel_pressed_last) >= 0.5 or (fastMode and (cur_time-decel_pressed_last) >= 1)):
        v_cruise_kph -= 1
    else:
